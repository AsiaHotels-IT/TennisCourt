...
const AuditBooking = () => {
  ...
  const [paymentType, setPaymentType] = useState('QR'); // QR
  ...
  const handleDeleteReservation = async (reservID, name) => {
    ...
    try {
      await deleteReservations(reservID, name); 
      const res = await getReservations();
      const mappedEvents = mapReservationsToEvents(res.data);
      setEvents(mappedEvents);
  ...
  return (
    <div className='booking-container'>
      ...
      >
        ...
        <span style={{textAlign: 'left'}}>{user ? `ยินดีต้อนรับ, ${user.name}` : 'กรุณาเข้าสู่ระบบ'}</span>
        {/* Right Controls */}
        <div
          style={{
            display: 'flex',
        ...
        >
          ...
          <button
            onClick={() => navigate("/auditVatReport")}
            style={buttonStyle}
          >
            รายงานภาษี
          </button>
      ...
      <div style={{ display:'flex', flexDirection:'row' }}>
        ...
        <div style={{ 
          ...
          {isPastDate(selectedDate) ? (
            ...
            <AuditReservation selectedDate={formatDate(selectedDate)} />
          )}
        </div>
      </div>
    </div>
